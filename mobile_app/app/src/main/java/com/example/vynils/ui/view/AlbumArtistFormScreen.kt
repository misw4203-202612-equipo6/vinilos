...
import com.example.vynils.ui.viewmodel.AlbumArtistFormViewModel
import com.example.vynils.ui.viewmodel.ArtistOption
import com.example.vynils.ui.viewmodel.ArtistType

@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun AlbumArtistFormScreen(
    navController: NavController,
    albumId: Int,
    viewModel: AlbumArtistFormViewModel = viewModel()
) {
    val state by viewModel.state.collectAsState()
    var expanded by remember { mutableStateOf(false) }

    LaunchedEffect(albumId) {
        viewModel.loadArtists(albumId)
    }
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text(stringResource(id = R.string.title_add_artist_to_album)) },
                navigationIcon = {
        ...
        Column(
            ...
            if (state.loading) {
                ...
                Column(modifier = Modifier.fillMaxWidth()) {
                    OutlinedTextField(
                        value = state.selectedArtist?.let { artistLabel(it) }
                ...
                if (state.artists.isEmpty()) {
                    Text(
                        text = stringResource(id = R.string.msg_all_artists_assigned),
            ...
            Button(
                onClick = { viewModel.saveArtist(albumId) },
...
@Composable
private fun artistLabel(artist: ArtistOption): String {
    return when (artist.type) {
        ArtistType.BAND -> stringResource(id = R.string.label_band_suffix, artist.name)
        ArtistType.MUSICIAN -> stringResource(id = R.string.label_musician_suffix, artist.name)
    }
}
